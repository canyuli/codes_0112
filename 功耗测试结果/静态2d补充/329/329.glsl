...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    // 2. 计算SDF距离
    // INSERT HERE
    primitive_dist=sdArc(uv,0.000,0.186,1.581,0.005,0.671,0.066);
    d=opUnion(d, primitive_dist);
    primitive_dist=sdJoint2DSphere(uv,0.668,-0.065,0.099,0.000,4.722,0.234);
    d=opUnion(d, primitive_dist);
    primitive_dist=sdJoint2DSphere(uv,-0.669,-0.064,0.099,0.000,4.699,0.234);
    d=opUnion(d, primitive_dist);
    primitive_dist=sdCircle(uv, 0.179,-0.741,0.178);
    d=opUnion(d, primitive_dist);
    primitive_dist=sdJoint2DSphere(uv,0.669,-0.062,0.104,0.000,4.712,0.103);
    d=opSubtraction(d, primitive_dist);
    primitive_dist=sdJoint2DSphere(uv,-0.669,-0.062,0.104,0.000,4.713,0.103);
    d=opSubtraction(d, primitive_dist);
    primitive_dist=sdArc(uv,0.307,-0.410,0.900,2.266,0.333,0.063);
    d=opUnion(d, primitive_dist);
    primitive_dist=sdCircle(uv, 0.178,-0.741,0.050);
    d=opSubtraction(d, primitive_dist);
    // 3. 抗锯齿上色（基于最终 d）
