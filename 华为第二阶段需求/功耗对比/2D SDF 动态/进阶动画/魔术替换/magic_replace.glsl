...
void main() {
    ...
    if (uSwitch){
        if(phase1 < 1.0){
            scale2 = 0;
            scale3 = mix(1.0, 0.6, phase1);
        } else {
            float bounce = clamp(easeOutBack(phase2), 0.0, 1.0);
            scale2 = mix(0.6, 1.0, bounce);
            scale3 = 0;
        }
    }
    else{
        if(phase1 < 1.0){
            scale2 = mix(1.0, 0.6, phase1);
            scale3 = 0;
        } else {
            float bounce = clamp(easeOutBack(phase2), 0.0, 1.0);
            scale2 = 0;
            scale3 = mix(0.6, 1.0, bounce);
        }
    }
    vec2 uv2 = (uv - uCenter) / max(scale2, 1e-3) + uCenter;
    float sd1 = texture(uSDF[1], uv2).r - 0.5;
    sd1 = -sd1;
    vec2 uv3 = (uv - uCenter) / max(scale3, 1e-3) + uCenter;
    float sd2 = texture(uSDF[2], uv3).r - 0.5;
    sd2 = -sd2;
    float circleScale = max(scale2, scale3);
    float sd3 = sdCircle(uv, uCenter.x, uCenter.y, uRadius * circleScale);
    ...
    float sd = opSubtraction(sd0, sd3);
    sd = opUnion(sd, sd1);
    sd = opUnion(sd, sd2);
