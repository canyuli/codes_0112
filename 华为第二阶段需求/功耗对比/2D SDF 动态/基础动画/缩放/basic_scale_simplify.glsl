...
void main(){
    ...
    float scale = 0.5;
    if(iTime < aTime * 0.4){
        float t = smoothstep(0.0, 1.0, iTime / (aTime * 0.4));
        scale = mix(0.5, 0.65, t);
    }else if(iTime < aTime * 0.6){
        scale = 0.65;
    }else if(iTime < aTime){
        float t = smoothstep(0.0, 1.0, (iTime - 0.6 * aTime) / (aTime * 0.4));
        scale = mix(0.65, 0.5, t);
    }
