...
void getSQ(int i, out float params[11])
{
    if (i == 0) {
        const float tmp[11] = float[11](
            9.3496e-01, 1.0047e+00, 1.4921e-01,
            2.5455e-01, 1.4678e-01, 1.8144e-11,
            7.2632e-01, -1.0512e-11, -1.1930e+00,
            6.9930e-03, 6.9930e-03
        );
        for (int k = 0; k < 11; ++k) params[k] = tmp[k];
    }
    else if (i == 1) {
        const float tmp[11] = float[11](
            2.0435e-01, 9.9846e-01, 2.4565e-01,
            2.4566e-01, 7.1306e-01, 7.5619e-06,
            -1.2794e+00, -1.5708e+00, -1.1930e+00,
            6.9928e-03, 6.9819e-03
        );
        for (int k = 0; k < 11; ++k) params[k] = tmp[k];
    }
    else if (i == 2) {
        const float tmp[11] = float[11](
            9.8216e-01, 9.9070e-01, 2.4013e-01,
            2.3452e-01, 2.3938e-01, 6.8048e-02,
            4.7173e+00, -6.7909e-02, -1.1930e+00,
            -5.6917e-01, 7.0006e-03
        );
        for (int k = 0; k < 11; ++k) params[k] = tmp[k];
    }
    else if (i == 3) {
        const float tmp[11] = float[11](
            6.8465e-01, 9.9750e-01, 2.4549e-01,
            2.4548e-01, 3.6124e-01, 3.1416e+00,
            1.2968e-01, 1.5709e+00, -1.1930e+00,
            4.4274e-01, 7.0018e-03
        );
        for (int k = 0; k < 11; ++k) params[k] = tmp[k];
    }
    else {
