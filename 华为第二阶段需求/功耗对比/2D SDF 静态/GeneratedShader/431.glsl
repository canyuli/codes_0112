...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    d=opUnion(d, primitive_dist);
    primitive_dist=sdCapsule(uv,0.751,0.002,0.086,3.149,0.063);
    d=opUnion(d, primitive_dist);
    primitive_dist=sdCapsule(uv,0.002,-0.752,0.084,4.720,0.063);
    d=opUnion(d, primitive_dist);
    primitive_dist=sdCapsule(uv,-0.001,0.751,0.086,4.718,0.063);
    d=opUnion(d, primitive_dist);
    primitive_dist=sdCapsule(uv,-0.752,-0.001,0.084,3.144,0.064);
    d=opUnion(d, primitive_dist);
    // 3. 抗锯齿上色（基于最终 d）
