...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    // 2. 计算SDF距离
    // INSERT HERE
    primitive_dist=sdCircle(uv, -0.000,0.000,0.921);
    d=opUnion(d, primitive_dist);
    primitive_dist=sdCircle(uv, -0.000,0.000,0.791);
    d=opSubtraction(d, primitive_dist);
    primitive_dist=sdJoint2DSphere(uv,-0.005,0.017,0.446,0.000,4.713,0.067);
    d=opUnion(d, primitive_dist);
    primitive_dist=sdJoint2DSphere(uv,-0.244,0.245,0.297,0.000,2.360,0.065);
    d=opUnion(d, primitive_dist);
    primitive_dist=sdJoint2DSphere(uv,0.229,0.245,0.295,0.000,3.928,0.065);
    d=opUnion(d, primitive_dist);
    // 3. 抗锯齿上色（基于最终 d）
