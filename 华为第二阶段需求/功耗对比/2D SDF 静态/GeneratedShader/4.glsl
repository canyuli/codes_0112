...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    // 2. 计算SDF距离
    // INSERT HERE
    primitive_dist=sdIsoscelesTriangle(uv,0.397,0.456,0.642,0.318,1.571,0.133);
    d=opUnion(d, primitive_dist);
    primitive_dist=sdIsoscelesTriangle(uv,0.399,-0.454,0.651,0.322,1.571,0.131);
    d=opUnion(d, primitive_dist);
    primitive_dist=sdJoint2DSphere(uv,-0.193,-0.229,0.382,0.000,2.360,0.067);
    d=opUnion(d, primitive_dist);
    primitive_dist=sdJoint2DSphere(uv,-0.217,0.254,0.347,0.000,3.925,0.067);
    d=opUnion(d, primitive_dist);
    primitive_dist=sdIsoscelesTriangle(uv,0.400,0.456,0.648,0.328,1.566,0.001);
    d=opSubtraction(d, primitive_dist);
    primitive_dist=sdIsoscelesTriangle(uv,0.398,-0.454,0.643,0.320,1.568,0.003);
    d=opSubtraction(d, primitive_dist);
    // 3. 抗锯齿上色（基于最终 d）
