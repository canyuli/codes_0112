...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    // 2. 计算SDF距离
    // INSERT HERE
    primitive_dist=sdRectangle(uv,-0.133,0.000,1.062,1.658,4.712,0.242);
    d=opUnion(d, primitive_dist);
    primitive_dist=sdRectangle(uv,-0.136,0.000,0.788,1.400,4.712,0.082);
    d=opSubtraction(d, primitive_dist);
    primitive_dist=sdRectangle(uv,-0.310,0.000,0.637,0.902,4.712,-0.161);
    d=opUnion(d, primitive_dist);
    primitive_dist=sdJoint2DSphere(uv,0.876,0.000,0.165,0.000,1.571,0.084);
    d=opUnion(d, primitive_dist);
    // 3. 抗锯齿上色（基于最终 d）
