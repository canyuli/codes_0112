...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    // 2. 计算SDF距离
    // INSERT HERE
    primitive_dist=sdRectangle(uv,0.001,-0.166,1.255,1.753,4.713,0.252);
    d=opUnion(d, primitive_dist);
    primitive_dist=sdRectangle(uv,-0.216,-0.094,0.119,2.492,4.712,0.000);
    d=opSubtraction(d, primitive_dist);
    primitive_dist=sdRectangle(uv,0.001,0.501,0.592,1.002,4.713,0.257);
    d=opUnion(d, primitive_dist);
    primitive_dist=sdRectangle(uv,0.001,0.562,0.203,0.738,4.712,0.054);
    d=opSubtraction(d, primitive_dist);
    primitive_dist=sdJoint2DSphere(uv,0.000,-0.093,0.144,0.000,3.141,0.114);
    d=opSubtraction(d, primitive_dist);
    // 3. 抗锯齿上色（基于最终 d）
