...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    // 2. 计算SDF距离
    // INSERT HERE
    primitive_dist=sdCircle(uv, -0.002,-0.001,0.724);
    d=opUnion(d, primitive_dist);
    primitive_dist=sdCircle(uv, -0.001,-0.001,0.595);
    d=opSubtraction(d, primitive_dist);
    primitive_dist=sdCircle(uv, -0.000,-0.000,0.332);
    d=opUnion(d, primitive_dist);
    primitive_dist=sdJoint2DSphere(uv,0.751,0.002,0.086,0.000,3.149,0.063);
    d=opUnion(d, primitive_dist);
    primitive_dist=sdJoint2DSphere(uv,0.002,-0.752,0.084,0.000,4.720,0.063);
    d=opUnion(d, primitive_dist);
    primitive_dist=sdJoint2DSphere(uv,-0.001,0.751,0.086,0.000,4.718,0.063);
    d=opUnion(d, primitive_dist);
    primitive_dist=sdJoint2DSphere(uv,-0.752,-0.001,0.084,0.000,3.144,0.064);
    d=opUnion(d, primitive_dist);
    // 3. 抗锯齿上色（基于最终 d）
