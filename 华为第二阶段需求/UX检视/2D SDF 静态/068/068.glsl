...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    // 2. 计算SDF距离
    // INSERT HERE
    primitive_dist=sdRectangle(uv,0.408,0.409,0.755,0.759,3.927,0.241);
    d=opUnion(d, primitive_dist);
    primitive_dist=sdRectangle(uv,-0.468,-0.468,0.756,0.756,4.712,0.236);
    d=opUnion(d, primitive_dist);
    primitive_dist=sdRectangle(uv,0.408,-0.468,0.756,0.757,4.712,0.238);
    d=opUnion(d, primitive_dist);
    primitive_dist=sdRectangle(uv,-0.468,0.408,0.757,0.756,4.713,0.238);
    d=opUnion(d, primitive_dist);
    primitive_dist=sdRectangle(uv,0.409,0.408,0.493,0.495,3.926,0.096);
    d=opSubtraction(d, primitive_dist);
    primitive_dist=sdRectangle(uv,-0.468,0.408,0.494,0.494,4.712,0.096);
    d=opSubtraction(d, primitive_dist);
    primitive_dist=sdRectangle(uv,-0.468,-0.468,0.494,0.494,3.142,0.096);
    d=opSubtraction(d, primitive_dist);
    primitive_dist=sdRectangle(uv,0.408,-0.468,0.494,0.494,3.142,0.096);
    d=opSubtraction(d, primitive_dist);
    // 3. 抗锯齿上色（基于最终 d）
