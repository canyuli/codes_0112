...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    // 2. 计算SDF距离
    // INSERT HERE
    primitive_dist=sdIsoscelesTrapezoid(uv,-0.001,-0.005,0.812,1.098,1.026,0.001,0.160);
    d=opUnion(d, primitive_dist);
    primitive_dist=sdJoint2DSphere(uv,0.001,-0.517,0.549,0.000,3.140,0.189);
    d=opSubtraction(d, primitive_dist);
    primitive_dist=sdCircle(uv, 0.519,-0.522,0.084);
    d=opUnion(d, primitive_dist);
    // 3. 抗锯齿上色（基于最终 d）
