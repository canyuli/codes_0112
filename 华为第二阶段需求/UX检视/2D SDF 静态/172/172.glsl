...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    // 2. 计算SDF距离
    // INSERT HERE
    primitive_dist=sdCircle(uv, -0.000,-0.000,0.920);
    d=opUnion(d, primitive_dist);
    primitive_dist=sdCircle(uv, -0.001,-0.413,0.186);
    d=opSubtraction(d, primitive_dist);
    primitive_dist=sdCircle(uv, 0.001,0.413,0.186);
    d=opSubtraction(d, primitive_dist);
    primitive_dist=sdCircle(uv, -0.413,0.001,0.186);
    d=opSubtraction(d, primitive_dist);
    primitive_dist=sdCircle(uv, 0.413,-0.001,0.186);
    d=opSubtraction(d, primitive_dist);
    primitive_dist=sdJoint2DSphere(uv,0.385,-0.850,0.398,0.000,3.155,0.066);
    d=opUnion(d, primitive_dist);
    // 3. 抗锯齿上色（基于最终 d）
