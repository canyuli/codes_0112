...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    // 2. 计算SDF距离
    // INSERT HERE
    primitive_dist=sdJoint2DSphere(uv,-0.000,0.283,0.217,0.000,4.712,0.335);
    d=opUnion(d, primitive_dist);
    primitive_dist=sdArc(uv,0.000,0.064,1.568,3.142,0.563,0.065);
    d=opUnion(d, primitive_dist);
    primitive_dist=sdJoint2DSphere(uv,0.000,0.286,0.222,0.000,1.571,0.205);
    d=opSubtraction(d, primitive_dist);
    primitive_dist=sdJoint2DSphere(uv,-0.000,-0.641,0.131,0.000,4.712,0.061);
    d=opUnion(d, primitive_dist);
    // 3. 抗锯齿上色（基于最终 d）
